...
///         dispatcher (ShieldedPoolDispatcher.yul in this same directory)
/// @notice FORK of soispoke/minimal-shielded-pool's contracts/src/ShieldedPoolLogic.sol,
///         pinned at lib/minimal-shielded-pool (commit fb02e3c). The only logic change from
///         upstream is in `_spend()`, marked below with "FORK:" — everything else,
///         including every comment reproduced from upstream, is unmodified. See
///         pool-issues.md at the repo root for what changed and why it's safe.
///
///         PoseidonBN254.sol / Groth16Verifier.sol / PoseidonT3.sol / PoseidonT4.sol are
///         vendored as byte-identical copies in this same directory rather than imported
///         from the submodule: empirically, solc's via-IR optimizer produces ~4KB MORE
///         runtime bytecode for this exact contract when those imports resolve outside
///         this directory (confirmed by isolating the effect — same content, same solc
///         flags, only the import's resolved path differs), which is enough to push the
///         deploy past Hegotá's EIP-7825 gas cap. Keeping every file in one directory,
///         matching upstream's own layout, avoids the cliff. See pool-issues.md.
///
///         The production-preferred split of the pool-as-sender design: the
...
///         consistent if ever called directly.
contract ShieldedPoolLogic {
    ...
    /// Stateless Yul envelope reader (EnvelopeProbe.yul): returns the
    ...
    /// FORK: upstream computes this as `keccak256(abi.encode(address(this), SALT))`,
    /// which ABI-encodes the address LEFT-PADDED to 32 bytes (a 64-byte preimage).
    /// ethrex's actual native RECENT_ROOT_ADDRESS write derives source_id as
    /// `keccak256(caller || salt)` over the RAW 20-byte caller address and the
    /// 32-byte salt (a 52-byte preimage) -- confirmed spec-conformant, not an
    /// ethrex divergence, per ethrex's own docs/eip-8272.md #5 ("EIP-8272
    /// Specification preamble fixes the encodings: addresses are 20 bytes").
    /// Upstream's padded version can never match a real natively-written entry,
    /// silently breaking every recent-root binding on a real ethrex node even
    /// though it's internally self-consistent enough to pass a mocked-predeploy
    /// test suite. `abi.encodePacked` gives the correct unpadded 52-byte preimage.
    /// See pool-issues.md.
    function sourceId() public view returns (bytes32) {
        return keccak256(abi.encodePacked(address(this), SALT));
    }
    ...
    /// the proof over the byte-exact settle-frame Spend (see the single-verification
    /// analysis in the pool's devnet/REVIEW.md and its narrow dependence on the
    ...
    function _spend(Spend calldata s) internal returns (address payer) {
        ...
        // FORK: upstream requires `frames == 2 && frameIndex == 1` (self-pay)
        // or `frames == 3 && frameIndex == 2` (paymaster) — i.e. the settle
        // frame must be the transaction's literal last frame. We additionally
        // accept a 4-frame self-pay shape (`frames == 4 && frameIndex == 1`)
        // so a DEFAULT frame (private-swap execution) and a POST_TX frame
        // (EIP-7906 assertion) may follow the settle frame. The direct-target
        // check (`settleTarget != address(this)`) is untouched: that's what
        // actually blocks the indirection/double-credit class of bug upstream's
        // security review found, and it still holds for the 4-frame case since
        // our trailing frames never call the pool again. See pool-issues.md.
