...
/// @notice FORK of soispoke/minimal-shielded-pool's devnet/ShieldedPoolDispatcher.yul,
///         pinned at lib/minimal-shielded-pool (commit fb02e3c). The only change from
///         upstream is one extra `case 4` arm in verifyFrameApprove()'s frame-count
///         switch, marked below with "FORK:" — everything else, including every
///         comment reproduced from upstream, is unmodified. See pool-issues.md at the
///         repo root for what changed and why it's safe.
///
///         The production-preferred pool-as-sender shell: a thin immutable Yul
...
///         implementation (ShieldedPoolLogic.sol, in this same directory).
...
///             verifies the proof over the byte-exact settle-frame Spend, and
...
///         in upstream's devnet/REVIEW.md, and it depends on THIS dispatcher's
///         frame-0 code and its verifier binding being immutable: both the
...
object "ShieldedPoolDispatcher" {
    ...
    object "runtime" {
        code {
            ...
            // FORK: upstream hashed pad32(address) || SALT (64 bytes). ethrex's
            // actual native RECENT_ROOT_ADDRESS write derives source_id from the
            // RAW 20-byte caller address || 32-byte salt (52 bytes, spec-conformant
            // per ethrex's docs/eip-8272.md #5) -- see the matching fix and comment
            // in ShieldedPoolLogic.sol's sourceId() and pool-issues.md. shl(96, ...)
            // left-aligns the address in the first 20 bytes of the word so the next
            // store (salt, at offset 0x14) sits immediately after it with no gap.
            ...
            function verifyFrameApprove() {
                ...
                // FORK: upstream only accepts `frames == 2` (self-pay) or
                // `frames == 3` (paymaster) — the settle frame must be the
                // transaction's literal last frame. We additionally accept a
                // 4-frame self-pay shape so a DEFAULT frame (private-swap
                // execution) and a POST_TX frame (EIP-7906 assertion) may
                // follow the settle frame at index 1. Every check below keys
                // off `settleIndex`/`selfPay`, not `frames` or "last frame", so
                // this is the only line that needed to change here — see
                // pool-issues.md.
                ...
                // settle frame: the only pool execution, directly into this pool
