...
export async function fundAndApproveForSwap(
  ...
  if (selfPay) {
    // Bare self_verify (no deploy frame -- the account already exists): the account mints
    // its own IN_TOKEN, then approves MockSwap, both paid from its own balance (left over
    // from provisionAccount's faucet claim -- Hegotá's ~7 wei base fee comfortably covers
    // both steps' gas from a single 1 ETH claim). Not atomic-batched: isAccountFunded's own
    // balance>0 && allowance>0 check already tolerates a partial outcome (mint succeeds,
    // approve doesn't) by simply reporting "not funded yet", leaving this retriable.
    ...
    const result = await submitFrameTx(provider, plan, autoWalletSigner(signer.address));
    if (result.outcome !== "success") {
      throw new Error(`Fund+approve ${result.outcome} (tx ${result.txHash})`);
    }
    return { fundTxHash: result.txHash, approveTxHash: result.txHash };
  }
